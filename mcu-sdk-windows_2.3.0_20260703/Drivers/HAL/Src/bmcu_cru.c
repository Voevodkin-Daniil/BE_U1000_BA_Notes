...
#define IS_CRU_PORT(__VALUE__)                  (((__VALUE__) == CRU_PORT_A) || \
                                                 ((__VALUE__) == CRU_PORT_B) || \
                                                 ((__VALUE__) == CRU_PORT_C))

#define IS_CRU_PIN(__VALUE__)                   ((__VALUE__) != 0U)

#define IS_CRU_PIN_PULL(__VALUE__)              (((__VALUE__) == CRU_PIN_PULL_NO) || \
                                                 ((__VALUE__) == CRU_PIN_PULL_UP) || \
                                                 ((__VALUE__) == CRU_PIN_PULL_DOWN) || \
                                                 ((__VALUE__) == CRU_PIN_PULL_UP_DOWN))

#define IS_CRU_PIN_DRIVE_STRENGTH(__VALUE__)    (((__VALUE__) == CRU_PIN_DRIVE_STRENGTH_0) || \
                                                 ((__VALUE__) == CRU_PIN_DRIVE_STRENGTH_1) || \
                                                 ((__VALUE__) == CRU_PIN_DRIVE_STRENGTH_2) || \
                                                 ((__VALUE__) == CRU_PIN_DRIVE_STRENGTH_3))

#define IS_CRU_PIN_AF(__VALUE__)                (((__VALUE__) == CRU_PIN_AF_0) || \
                                                 ((__VALUE__) == CRU_PIN_AF_1) || \
                                                 ((__VALUE__) == CRU_PIN_AF_2) || \
                                                 ((__VALUE__) == CRU_PIN_AF_3) || \
                                                 ((__VALUE__) == CRU_PIN_AF_4) || \
                                                 ((__VALUE__) == CRU_PIN_AF_5))
...
/**
 * @brief Initializes CRU settings of a GPIO pin.
 * @note Initialization of only one pin allowed at time.
 * @note CRU_PIN_InitStruct structure should be initialized prior to calling this function.
 * @param CRU_PIN_InitStruct The pointer to CRU_PIN_InitStruct_TypeDef structure.
 * @retval An ErrorStatus enumeration value:
 *         - SUCCESS: The pin have been initialized.
 *         - ERROR: The pin have not been initialized.
 */
ErrorStatus CRU_PIN_Init(CRU_PIN_InitStruct_TypeDef *CRU_PIN_InitStruct)
{
    ...
    /* Check parameters */
    assert(IS_CRU_PORT(CRU_PIN_InitStruct->Port));
    assert(IS_CRU_PIN(CRU_PIN_InitStruct->Pin));
    assert(IS_CRU_PIN_PULL(CRU_PIN_InitStruct->Pull));
    assert(IS_FUNCTIONAL_STATE(CRU_PIN_InitStruct->InputCtrl));
    assert(IS_CRU_PIN_DRIVE_STRENGTH(CRU_PIN_InitStruct->DriveStrength));
    assert(IS_CRU_PIN_AF(CRU_PIN_InitStruct->Alternate));

    if (CRU_PIN_InitStruct->Pin == (1U << POSITION_VAL(CRU_PIN_InitStruct->Pin)))
    {
        /* Configure pin pull-up or pull-down */
...
/**
 * @brief Sets each field of CRU_PIN_InitStruct_TypeDef structure to default value.
 * @note Initialization of only one pin allowed at time.
 * @param CRU_PIN_InitStruct The pointer to CRU_PIN_InitStruct_TypeDef structure.
 */
void CRU_PIN_StructInit(CRU_PIN_InitStruct_TypeDef *CRU_PIN_InitStruct)
{
    /* Set CRU_PIN_InitStruct fields to default values */
    CRU_PIN_InitStruct->Port = CRU_PORT_A;
    CRU_PIN_InitStruct->Pin = CRU_PIN_0;

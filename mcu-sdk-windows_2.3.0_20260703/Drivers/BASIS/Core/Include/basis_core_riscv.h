...
/**
 * @brief Returns hardware thread ID.
 * @note Can only be executed in Machine mode.
 * @returns The hardware thread ID.
 */
__STATIC_INLINE unsigned long __get_hart_id(void)
{
    return (CSR_READ(CSR_MHARTID) & 0x3FFUL);
}
 ...
 * @note    Can be executed in User mode or higher privilege mode.
 * @note    t0 - "cycles per microisecond" immediate value
 *          t1 - 1 us start timestamp value
 *          t2 - the current timer value
...
void __delay_us(unsigned long us)
{
    __ASM volatile (
        ...
        "3:                         \n"
        : [arg]"+r"(us)
        : [cycles_per_us]"i"(TIME_CYCLES_PER_US)
        : "t0", "t1", "t2"
    );
}
 ...
 *          F_TIMER_PULSE frequency and synchronous with CCLK.
 * @note    Can be executed in User mode or higher privilege mode.
 * @note    t0 - "cycles per millisecond" immediate value
 *          t1 - 1 ms start timestamp value
 *          t2 - the current timer value
...
void __delay_ms(unsigned long ms)
{
    __ASM volatile (
        ...
        "3:                         \n"
        : [arg]"+r"(ms)
        : [cycles_per_ms]"i"(TIME_CYCLES_PER_MS)
        : "t0", "t1", "t2"
    );
}
...
/**
 * @brief Enter the ISR that supports nesting.
 * @note Can only be executed in Machine mode.
 * @note This macro should always be used in pair with EXIT_NESTED_INTERRUPT().
 */
#define ENTER_NESTED_INTERRUPT()                    ({                                                      \
                                                        __ASM volatile (                                    \
                                                            "addi sp, sp, -8    \n"                         \
                                                            "csrr t0, mcause    \n"                         \
                                                            "sw t0, 0(sp)       \n"                         \
                                                            ...
                                                            "csrsi mstatus, %0  \n"                         \
                                                            :                                               \
                                                            : "i"(CSR_MSTATUS_MIE_Msk)                      \
                                                            : "t0", "memory"                                \
                                                        );                                                  \
                                                    })

/**
 * @brief Exit the ISR that supports nesting.
 * @note Can only be executed in Machine mode.
 * @note This macro should always be used in pair with ENTER_NESTED_INTERRUPT().
 */
#define EXIT_NESTED_INTERRUPT()                     ({                                                      \
                                                        __ASM volatile (                                    \
                                                            "lw t0, 4(sp)       \n"                         \
                                                            "csrw mepc, t0      \n"                         \
                                                            "lw t0, 0(sp)       \n"                         \

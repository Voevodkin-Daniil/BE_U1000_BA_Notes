/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/GPIO/GPIO_InputEXTI/main.c
 *  @author     Baikal electronics SDK team
 *  @brief      GPIO input EXTI interrupt mode example source file
 *  @version    2.3.0
 *  @date       20260703
 * *****************************************************************************
...
btn_init (void) {
    ...
    /* Configure EXTI interrupt for the user button pin */
    CRU_C0_C1_EXTI_SetSource(BTN_CRU_PORT, BTN_CRU_PIN);

    /* Configure CLIC for the GPIO interrupt
       Pay attention to "Type" and "Polarity" parameters
       These parameters control the user button EXTI interrupt triggering mode */
    CLIC_ConfigIRQ(BTN_CLIC_IRQN,                   /* Interrupt */
                   CLIC_INTATTR_MODE_MACHINE,       /* Privilege mode */
                   1U,                              /* Level */
                   1U,                              /* Priority */
                   CLIC_INTATTR_SHV_VECTORED,       /* Vector mode */
                   CLIC_INTATTR_TRIG_TYPE_EDGE,     /* Type: LEVEL or EDGE trigger */
                   CLIC_INTATTR_TRIG_POL_N);        /* Polarity: (P)ositive or (N)egative edge trigger */
...
is configured as an EXTI interrupt source */

/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/Benchmarks/USB/Device/USB_CDC_TestBW_TX/main.c
...
static uint8_t cdc_buf[CFG_TUD_CDC_EP_BUFSIZE];
...
cdc_task (void) {
    ...
    if (run_once)
    {
        ...
        {
            tud_cdc_read(cdc_buf, CFG_TUD_CDC_EP_BUFSIZE);

            if (tud_cdc_write_available() >= CFG_TUD_CDC_EP_BUFSIZE)
            {
                memset(cdc_buf, '#', CFG_TUD_CDC_EP_BUFSIZE);
                tud_cdc_write(cdc_buf, CFG_TUD_CDC_EP_BUFSIZE);
            }

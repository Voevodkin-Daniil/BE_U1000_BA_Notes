/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/Benchmarks/USB/Device/USB_DFU_TestBW/dfu.c
...
tud_dfu_download_cb (uint8_t alt,
    ...
    switch (mem_dst) {
        case DST_TCM:
            memcpy((uint8_t *)(BASE_ADDR_TCM + dfu_state_s.dnload_cnt[mem_dst] % MEM_SIZE_TCM),
                    data, length);
        ...
        case DST_EFLASH:
            while (data_cnt < length) {
                ...
                EFLASH_ProgramWord(BASE_ADDR_EFLASH + dfu_state_s.dnload_cnt[mem_dst] % MEM_SIZE_EFLASH,
...
tud_dfu_upload_cb(uint8_t alt,
    ...
    switch (mem_dst) {
        case DST_TCM:
            xfer_len = ((MEM_SIZE_TCM_VIRT - dfu_state_s.upload_cnt[mem_dst]) >= length) ? \
            length : (MEM_SIZE_TCM_VIRT - dfu_state_s.upload_cnt[mem_dst]);
            memcpy(data, (uint8_t *)(BASE_ADDR_TCM + dfu_state_s.upload_cnt[mem_dst] % MEM_SIZE_TCM), xfer_len);
            dfu_state_s.upload_cnt[mem_dst] += xfer_len;
            break;

        case DST_EFLASH:
            xfer_len = ((MEM_SIZE_EFLASH_VIRT - dfu_state_s.upload_cnt[mem_dst]) >= length) ? \
            length : (MEM_SIZE_EFLASH_VIRT - dfu_state_s.upload_cnt[mem_dst]);
            
            xfer_len &= ~(sizeof(uint32_t) - 1);
            
            for (uint32_t i = 0; i < xfer_len; i += sizeof(uint32_t)) {
                EFLASH_ReadWord(BASE_ADDR_EFLASH + dfu_state_s.upload_cnt[mem_dst] % MEM_SIZE_EFLASH,

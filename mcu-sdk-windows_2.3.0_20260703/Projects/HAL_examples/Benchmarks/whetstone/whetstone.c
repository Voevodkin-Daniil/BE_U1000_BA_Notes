...
#define COMPILER_VERSION    "GCC"__VERSION__    /**< Compiler version */
#define REPORT_PC_MODEL     "SoC"               /**< PC model */
#define REPORT_CPU          "BMCU"              /**< CPU */

#define REPORT_MATH_LIB     xstr(MATH_VER)      /**< Used math library */

#ifndef OPT
#error "OPT is undefined"
#else

#if OPT==4
#define REPORT_OPT          "s"
#elif OPT==5
#define REPORT_OPT          "fast"
#else
#define REPORT_OPT          xstr(OPT)           /**< Optimization level */
#endif
...
#else
#define REPORT_CFLAGS xstr(CFLAGS)
#endif
...
static SPDP mwips, mwips_Per_MHz;
...
int main_whetstone()
{
    ...
    long x100 = 100;
    int duration = 10;
    SPDP Vax_Mips,Vax_Mips_Per_MHz, timea,timec,dtime();
    SPDP x, sinx, cosx, sqrtx,expx, log10fx, logfx;

#if (OPT != 0) && (MEM_REG_ROM != 0)
    x = 3.14/4;
    sinx = sinf(x);
    cosx = cosf(x);
    PRINTF("x %d sin %d cos %d *100\r\n",(int)(100.0*x), (int)(100.0*sinx) , (int)(100.0*cosx));
    x = 4;
    sqrtx = sqrtf(x);
    expx = expf(x);
    PRINTF("x %d sqrt %d exp %d *100\r\n",(int)(100.0*x), (int)(100.0*sqrtx) , (int)(100.0*expx));
    x = 100;
    log10fx = log10f(x);
    logfx = logf(x);
    PRINTF("x %d log %d log10f %d *100\r\n",(int)(100.0*x), (int)(100.0*logfx) , (int)(100.0*log10fx));
    ...
    PRINTF("10^9 NOPs time is %d.%03d, Sec\r\n", (int)timea, abs((int)((timea - (int)timea)*1000)));
#endif
    ...
    PRINTF("TimeUsed for pass = %d\r\n",(int)TimeUsed);

    TimeUsed = 0;

#ifdef XTRA
    xtra = XTRA; //Force set ITERATION# for whetstone
#endif
    ...
    PRINTF("whetstones time is %d, mSec \r\n", (int)(timec*1000));

    User_Time = End_Time - Begin_Time;

    PRINTF("[TimeUsed*1000]                             %d\r\n", (int)(TimeUsed*1000));
    ...
    Vax_Mips = mwips * 1.02;
    Vax_Mips_Per_MHz = Vax_Mips / (float) CLK_CCLK;
    mwips_Per_MHz = mwips / (float) CLK_CCLK;

#if (OPT != 0) && (MEM_REG_ROM != 0)
    PRINTF("MWIPS:                                      %d.%03d\r\n", (int)mwips, abs((int)((mwips - (int)mwips)*1000)));
    PRINTF("MWIPS per MHz:                              %d.%03d\r\n", (int)mwips_Per_MHz, abs((int)((mwips_Per_MHz - (int)mwips_Per_MHz)*1000)));

    PRINTF("VAX MIPS rating:                            %d.%03d\r\n", (int)Vax_Mips, abs((int)((Vax_Mips - (int)Vax_Mips)*1000)));
    PRINTF("VAX MIPS per MHz:                           %d.%03d\r\n", (int)Vax_Mips_Per_MHz, abs((int)((Vax_Mips_Per_MHz - (int)Vax_Mips_Per_MHz)*1000)));

    PRINTF("[DURATION]                                  %d.%03d\r\n", (int)TimeUsed, abs((int)((TimeUsed - (int)TimeUsed)*1000)));
    PRINTF("[ITERATIONS]                                %d\r\n", xtra*x100);
    PRINTF("[xtra]                                      %d\r\n", xtra);
    PRINTF("[x100]                                      %d\r\n", x100);
#else
    ...
    SPDP time, int calibrate, int section)
{
    ...
    if (calibrate == 0)
    {
        debug_printf("%s %24.17f    ", headings[section], results[section]);

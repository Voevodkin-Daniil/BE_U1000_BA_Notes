...
#error "Please define a way to initialize a memory block."
...
#else
    ticks_strt = get_ticks();
    iterate(&results[0]);
    ticks_stp = get_ticks();
    stop_time();

    PRINTF("Coremark iterate start tick counter: %lu\r\r\n", ticks_strt);
    PRINTF("Coremark iterate stop  tick counter: %lu\r\r\n", ticks_stp);
#endif
    total_time = get_time();
    ...
    uint32_t cclk_mhz = CLK_GetCCLKFreq() / 1000000;
    PRINTF("Core clock (MHz) : %lu\r\n", (long unsigned)cclk_mhz);

#if HAS_FLOAT
    float t = time_in_secs(total_time);
    PRINTF("Total time (secs): %d.%03d\r\n", (int)t, abs((int)((t - (int)t)*1000)));

    if (time_in_secs(total_time) > 0)
    {
        float is = (float) default_num_contexts * (float) results[0].iterations
            / (float) time_in_secs(total_time);
        PRINTF("Iterations/Sec   : %d.%03d\r\n", (int)is, abs((int)((is - (int)is)*1000)));

        float cr = is / (float) cclk_mhz;
        PRINTF("Coremarks/Mhz    : %d.%03d\r\n", (int)cr, abs((int)((cr - (int)cr)*1000)));
    }
#else
...
#if 0
// TODO: This is a known issue with PRINTF() when trying to use the "%f" format
#if HAS_FLOAT
        if (known_id == 3)
        {

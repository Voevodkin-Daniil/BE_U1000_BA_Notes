/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/I2S/I2S_Slave_TX_NormalModeIT/main.c
...
/* Interrupt flags are set in interrupt handlers */
volatile uint8_t i2s_tx_fifo_empty_it_flag = 0;
...
int main(void)
{
    ...
    for(;;)
    {
        ...
        while(wav_data_size)
        {
            /* 3.2 Wait TX FIFO is empty */
            while (!i2s_tx_fifo_empty_it_flag);

            /* 3.3 Add data to TX FIFO */
            while(I2S_IsActiveIT_TXFE(I2S1) == 1UL && wav_data_size != 0)
            {
                I2S_TransmitDataLeft(I2S1, wav_massive[wav_data_idx++]);
                I2S_TransmitDataRight(I2S1, wav_massive[wav_data_idx++]);
                wav_data_size-=2;
            ...
            /* 3.4 Clear the TX FIFO flag Empty interrupt */
            i2s_tx_fifo_empty_it_flag = 0;
            I2S_EnableIT_TXFE(I2S1);
        }
...
void __attribute__ ((interrupt)) I2S1_IRQHandler(void)
{
    ...
    if (status & I2S_ISR0_TXFE)
    {
        /* A TX FIFO Empty interrupt is cleared by writing
           data to the TX FIFO to bring its level above
           the empty trigger threshold level for the channel. */
        i2s_tx_fifo_empty_it_flag = 1;
        I2S_DisableIT_TXFE(I2S1);
    }

    if (status & I2S_ISR0_TXFO)
    {
        /* A Data Overrun interrupt is cleared by reading the
        Transmit Channel Overrun (TXCHO) bit [0] of the
        Transmit Overrun Register (TOR0). */
        I2S_ClearIT_TXFO(I2S1);
    }
}

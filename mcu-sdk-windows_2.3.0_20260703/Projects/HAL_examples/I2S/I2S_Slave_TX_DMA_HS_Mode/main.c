/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/I2S/I2S_Slave_TX_DMA_HS_Mode/main.c
...
__ALIGNED(4)
int16_t sinus_table[48] ={
      0xAAAA,   3930,   7791,  11512,  15028,  18276,  21198,  23741,  25861,  27519,
  28686,  29342,  29473,  29079,  28166,  26750,  24857,  22520,  19781,  16689,
  13300,   9673,   5874,   1969,  -1969,  -5874,  -9673, -13300, -16689, -19781,
 -22520, -24857, -26750, -28166, -29079, -29473, -29342, -28686, -27519, -25861,
 -23741, -21198, -18276, -15028, -11512,  -7791,  -3930,      0xA5A5};
...
void DMA1_Init (void)
{
    ...
    DMA1_InitStruct.Direction = DMA_DIR_MEMORY_TO_PERIPH;
    DMA1_InitStruct.FIFOMode = DMA_FIFO_MODE_0;
    DMA1_InitStruct.SrcHWHandshakeInterface = 0;
    DMA1_InitStruct.SrcHandshake = DMA_HS_HARDWARE;
    DMA1_InitStruct.SrcAddress = (uint32_t)sinus_table;
    DMA1_InitStruct.SrcIncMode = DMA_INC_MODE_INCREMENT;
    DMA1_InitStruct.SrcBurstLength = DMA_BURST_LENGTH_1;
    DMA1_InitStruct.SrcTransferWidth = DMA_TR_WIDTH_32BITS;

    DMA1_InitStruct.DstAddress = (uint32_t)&(I2S1->TXDMA) ;
    DMA1_InitStruct.DstHandshake = DMA_HS_HARDWARE;
    DMA1_InitStruct.DstHWHandshakeInterface = 4;
    DMA1_InitStruct.DstIncMode = DMA_INC_MODE_NO_CHANGE;
...
int main(void)
{
    ...
    for(;;)
    {
        ...
        printf("Enable DMA TX request\r\n");
        I2S_EnableDMAReq_TX(I2S1);

/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/I2S/I2S_Slave_TX_DMAMode/main.c
...
int main(void)
{
    ...
    for(;;)
    {
        ...
        /* 2. DMA mode example */
        /* Fill the TX-FIFO by writing to the Transmitter Block DMA Register (TXDMA). */
        while (I2S_IsActiveIT_TXFO(I2S1) == 0UL)
        {
            I2S_TransmitDMAData(I2S1, wav_massive[wav_data_idx++]);
            I2S_TransmitDMAData(I2S1, wav_massive[wav_data_idx++]);
            wav_data_size-=2;
        }

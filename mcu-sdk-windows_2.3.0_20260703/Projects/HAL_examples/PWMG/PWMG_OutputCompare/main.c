/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/PWMG/PWMG_OutputCompare/main.c
...
#define PWMG_OC_MODE_NUM            3U  /* The number of Output Compare modes */

/* Output Compare modes */
static const PWMG_OC_Mode_TypeDef OCMode[PWMG_OC_MODE_NUM] = {
    PWMG_OC_MODE_TOGGLE,            /* OC0REF toggles on compare match */
    PWMG_OC_MODE_FORCED_INACTIVE,   /* OC0REF is forced low */
    PWMG_OC_MODE_FORCED_ACTIVE      /* OC0REF is forced high */
};
...
PWMG1_Init (void)
{
    ...
    PWMG_InitStruct.Autoreload    = __PWMG_CALC_ARR(cru_clocks.PCLK2_Frequency, PWMG_InitStruct.Prescaler, 10U);
    ...
    PWMG_OC_InitStruct.OCMode       = OCMode[OCModeIndex];
    PWMG_OC_InitStruct.OCPolarity   = PWMG_OC_POLARITY_HIGH;
    PWMG_OC_InitStruct.OCState      = PWMG_OC_STATE_DISABLE;
    PWMG_OC_InitStruct.CompareValue = PWMG_GetAutoReload(PWMG1) / 2U;
    PWMG_OC_Init(PWMG1, &PWMG_OC_InitStruct);
...
main (void) {
    init();
    printf("PWMG Output Compare example, %s %s\r\n", __DATE__, __TIME__);
...
GPIO2_IRQHandler (void) {
    ...
    OCModeIndex = (OCModeIndex + 1U) % PWMG_OC_MODE_NUM;

    /* Disable counter */
    PWMG_DisableCounter(PWMG1);
  
    /* Reset counter */
    PWMG_SetCounter(PWMG1, 0U);
  
    /* Set counter mode */
    PWMG_OC_SetMode(PWMG1, OCMode[OCModeIndex]);

    if (OCMode[OCModeIndex] == PWMG_OC_MODE_FORCED_INACTIVE)
    {
    ...
    else if (OCMode[OCModeIndex] == PWMG_OC_MODE_FORCED_ACTIVE)
    {
        /* Forced active mode */
...
PWMG1_IRQHandler (void) {
    ...
    {
        if (OCMode[OCModeIndex] == PWMG_OC_MODE_TOGGLE)
        {
            /* Toggle LED */
            bsp_led_toggle();
        }

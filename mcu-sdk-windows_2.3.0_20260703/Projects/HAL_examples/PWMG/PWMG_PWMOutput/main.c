/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/PWMG/PWMG_PWMOutput/main.c
...
#define PWMG_DUTY_CYCLE_NUM             11U  /* The number of duty cycle values */

/* Duty cycle values */
static const uint8_t DutyCycle[PWMG_DUTY_CYCLE_NUM] = {
    0,      /* 0% */
    10,     /* 10% */
    20,     /* 20% */
    30,     /* 30% */
...
PWMG1_Init (void)
{
    ...
    PWMG_InitStruct.Prescaler     = __PWMG_CALC_PSC(cru_clocks.PCLK2_Frequency, 5000000U);
    PWMG_InitStruct.Autoreload    = __PWMG_CALC_ARR(cru_clocks.PCLK2_Frequency, PWMG_InitStruct.Prescaler, 100U);
    ...
    PWMG_OC_InitStruct.CompareValue = ((uint32_t)PWMG_GetAutoReload(PWMG1) * DutyCycle[DutyCycleIndex]) / 100U;
    PWMG_OC_Init(PWMG1, &PWMG_OC_InitStruct);

    /* Enable Capture/Compare register preload */
    PWMG_OC_EnablePreload(PWMG1);
...
main (void) {
    init();
    printf("PWMG PWM output example, %s %s\r\n", __DATE__, __TIME__);
...
GPIO2_IRQHandler (void) {
    ...
    DutyCycleIndex = (DutyCycleIndex + 1U) % PWMG_DUTY_CYCLE_NUM;
    PWMG_OC_SetCompare(PWMG1, ((uint32_t)PWMG_GetAutoReload(PWMG1) * DutyCycle[DutyCycleIndex]) / 100U);
}

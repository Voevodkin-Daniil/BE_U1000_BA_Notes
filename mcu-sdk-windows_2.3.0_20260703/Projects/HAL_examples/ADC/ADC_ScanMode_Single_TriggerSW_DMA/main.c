/**
 * *****************************************************************************
 *  @file       Projects/HAL_examples/ADC/ADC_ScanMode_Single_TriggerSW_DMA/main.c
...
void DMA2_Init(void)
{
    ...
    DMA_EnableIT(DMA2, DMA_CH2);

    /* Enable IntBlock interrupt */
    DMA_EnableIT_IntBlock(DMA2, DMA_CH2);

    /* Configure and enable DMA interrupt in CLIC */
    CLIC_ConfigIRQ(CLIC_DMA2_IRQn,                  /* Interrupt */
...
void __attribute__ ((interrupt)) DMA2_IRQHandler(void)
{
    ...
    /* Clear interrupt */
    DMA_ClearIT_IntBlock(DMA2, DMA_CH2);
}
